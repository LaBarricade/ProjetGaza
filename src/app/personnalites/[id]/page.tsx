...
export default async function PersonalityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto p-6 w-4xl ">
        ...
        <div className="ml-4 self-center mr-4">
          ...
          <ul className="flex flex-row gap-2 mt-3">
            {[p.social1_url, p.social2_url].map((social_url, i) => {
              ...
              else if (parsedUrl.hostname.includes('x.com')) icon = <TwitterIcon />;
              else if (parsedUrl.hostname.includes('facebook.com')) icon = <FacebookIcon />;
              else if (parsedUrl.hostname.includes('instagram.com')) icon = <InstagramIcon />;
              else icon = <span>{parsedUrl.hostname}</span>;

              return (
                <li>
                  <Link href={social_url} key={`social-${i}`}>
                    {icon}
                  </Link>
                </li>
              );
      ...
      {p.quotes.length > 3 && <QuotesTimeline quotes={p.quotes} />}

...
} from '@supabase-js/source/packages/core/postgrest-js/src';
...
class DbService {
  ...
  checkErrors(
    supabaseResp: PostgrestSingleResponse<any>
    ...
    else if (!supabaseResp.data) throw new Error('No data returned from supabase');
  }
  ...
  async findParties(): Promise<any> {
    ...
    const formattedData: Party[] = resp.data.map(
      (item: any): Party =>
        Object.assign(item, {
  ...
  async findPersonalities(params: {
    ...
    if (params.ids && params.ids.length > 0) query.in('id', params.ids);

    if (params.party && params.party.length > 0) query.in('parti_politique_id', params.party);

    if (params.role && params.role.length > 0) query.in('mandates.type_mandat_id', params.role);
  ...
  async findGeneralStats(): Promise<any> {
    const select = `min_date:date.min(), max_date:date.max()`;
  ...
  async findQuotes(params: any): Promise<any> {
    ...
    if (params.text) query.textSearch('citation', params.text, { type: 'websearch' });

    if (params.ids) query.in('id', params.ids);

...
export function PersonalityCard({ p }: { p: Personality }) {
  ...
  return (
    <Card
      className={`rounded-2xl shadow-md hover:shadow-lg transition overflow-hidden flex flex-col gap-1
                justify-between ${!p.quotes_count && 'opacity-50'}`}
    >
      <CardHeader className="px-4  ">
        <div className="flex flex-row justify-between gap-1">
          <div>
            <Link href={`/personnalites/${p.id}`} className="relative inline-block group -mx-2">
              {/* Background simple */}
              <span

...
export function FiltersBar({
  ...
  const { filters, setFilter, clearAllFilters, hasActiveFilters, countActiveFilters } =
    useSearchFilters({
      basePath: `/${pageName}`,
    });
  ...
  const vertSeparatorElement = (
    <Separator orientation="vertical" className="hidden lg:block h-16 opacity-50 " />
  );
  return (
    <div className="max-w-screen-lg mx-auto">
      ...
      <div className="bg-background z-10 p-6">
        <div
          className={`flex ${hasActiveFilters ? 'justify-between  items-center' : 'justify-center'}`}
        >
          {!alwaysVisible && (
            <button

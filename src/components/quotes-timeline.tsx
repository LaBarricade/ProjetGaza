...
export function QuotesTimeline({ quotes }: { quotes: Quote[] }) {
      ...
      const events = await Promise.all(
        quotes.map(async (q: Quote, i: number) => {
          ...
          if (q.source) {
            ...
            if (logo) logoCache[q.source.name] = logo;
          }
      ...
      if (window.TL && timelineRef.current) {
        ...
        setTimeout(() => {
          ...
          [...markers].forEach((marker) => {
            const source = (marker as HTMLElement).querySelector('.tl-headline')
            ...
            if (logo) {
              ...
              const headline: HTMLElement = (marker as HTMLElement).querySelector(
                '.tl-headline'
              ) as HTMLElement;
  ...
  return (
    <div ref={timelineRef} style={{ width: '100%', height: '500px', border: '1px solid #DDD' }} />
  );
}
